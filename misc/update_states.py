...
with open('data1.csv', newline='') as csvfile:
     ...
     for row in reader:
          if ", " in row["Province/State"]:
              ...
              all_data0[row["Province/State"].split(", ")[1].replace(" (From Diamond Princess)","")]={}
              dates=list(row.keys())[4:]
              for el in dates:
                   all_data0[row["Province/State"].split(", ")[1].replace(" (From Diamond Princess)","")][el]=[]
     ...
     for row in ddata:
          ...
          #print(row,dates2)
          for el in dates2:
               all_data0[row["Province/State"].split(", ")[1].replace(" (From Diamond Princess)","")][el].append(int(row[el]))
print(all_data0.keys())


#dict_keys(['WA', 'IL', 'AZ', 'ON', 'CA', 'MA', 'WI', 'TX', 'NE', 'QC', 'OR', 'RI', 'NH', 'FL', 'NY'])
kkeys={'WA':'Washington', 'IL':'Illinois', 'AZ':'Arizona', 'ON':'Ontario', 'CA':'California', 'MA':'Massachusetts', 'WI':'Wyoming', 'TX':'Texas', 'NE':'Nebraska', 'QC':'Quebec', 'OR':'Oregon','RI':'Rhode Island', 'NH':'New Hampshire','FL':'Florida','NY':'New York','GA':'Georgia','NC':'North Carolina','NJ':'New Jersey', 'NV':'Nevada', 'TN':'Tennessee','MD':'Maryland', 'CO':'Colorado', 'Alberta':'Alberta', 'PA':'Pennsylvania', 'KY':'Kentucky', 'IN':'Indiana', 'MN':'Minnesota', 'RI':'Phode Island', 'CT':'Connecticut', 'SC':'South Carolina', 'UT':'Utah', 'HI':'Hawaii', 'OR ':'Oregon', 'OR':'Oregon','OK':'Oklahoma','VA':'Virginia', 'D.C.':'District of Columbia', 'KS':'Kansas', 'MO':'Missouri', 'VT':'Vermont','LA':'Louisiana','IA':'Iowa'}
...
with open('ne_count18.json') as json_file:
    ...
    for elem in data["features"]:
         ...
         if elem["properties"]["name"] in list(all_data.keys()) and elem["properties"]["iso"] in ["US","CA"]:
...
print(json_file0)          
with open('ne_count14.json', 'w') as outfile:
